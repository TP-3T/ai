"""
Module for exporting a trained and tested GBDT model to .ONNX format.
"""
...
from skl2onnx import to_onnx, update_registered_converter # type: ignore
from skl2onnx.common.shape_calculator import ( # type: ignore
    calculate_linear_regressor_output_shapes, # type: ignore
)
from onnxmltools.convert.xgboost.operator_converters.XGBoost import convert_xgboost # type: ignore
import onnxruntime as onnxrt # type: ignore
from features.climate_prediction.constants.data_file_paths import GBDT_MODEL_FILE_NAME, GBDT_FEATURE_MAPPING_FILE_NAME # type: ignore
import json

def convert_feature_names_to_fx(dataset: DataFrame, save_mapping_to_json: bool = False) -> tuple[DataFrame, dict[str, str]]:
    """
    ...
    if save_mapping_to_json:
        with open(GBDT_FEATURE_MAPPING_FILE_NAME, 'w') as json_file:
...
def export_to_onnx(model: Pipeline, training_climate_dataset: DataFrame, testing_climate_dataset: DataFrame):
    ...
    print("gbdt Model in XGBRegressor format - predictions:", model.predict(test_features[:5])) # type: ignore
    ...
    # (which is a wrapper object containing the trained model)
    for _, step in model.steps:
        if isinstance(step, XGBRegressor):
            model_wrapper = step

    # get the underlying xgboost gbdt class from the sci-kit learn wrapper
    model_fx_names = model_wrapper.get_booster() # type: ignore

    model_fx_names.feature_names = fx_names

    model_onnx = to_onnx(
        model, 
        train_feat_fx.to_numpy(dtype=np.float32), # converts the multi dimensional dataframe to a numpy array equivalent with float32 types (no strings)
        target_opset={"": 12, "ai.onnx.ml": 2}
    )

    sess = onnxrt.InferenceSession(model_onnx.SerializeToString(), providers=["CPUExecutionProvider"]) # type: ignore
    pred_onx = sess.run(None, {"X": test_feat_fx[:5].to_numpy(dtype=np.float32)}) # type: ignore
    print("gbdt Model in ONNX format - predictions:", pred_onx[0].ravel()) # type: ignore

    # If this model output matches the one from above, the conversion was successful

    # Write the model content to a .ONNX file
    with open(GBDT_MODEL_FILE_NAME, "wb") as f:
        f.write(model_onnx.SerializeToString()) # type: ignore
